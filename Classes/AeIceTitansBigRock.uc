...
class	AeIceTitansBigRock	extends	SMPTitanBigRock;
...
simulated function PostBeginPlay()
{
	...
	if ( Level.NetMode != NM_DedicatedServer)
	{
		...
		SmokeTrail = Spawn(class'IceTitanFlames',self);
	}
	Dir = vector(Rotation);
	Velocity = speed * Dir;
    if ( Level.bDropDetail )
...
function ProcessTouch (Actor Other, Vector HitLocation)
{
	...
	if(Projectile(Other)!=none)
		Other.Destroy();
	else if ( !Other.IsA('AeTitanBigBigRock'))
	{
...
function SpawnChunks(int num)
{
	...
	{
		TempRock = Spawn(class'AeIceTitansBigRock');
		SmokeTrail = Spawn(class'IceTitanFlames',self);
		if (TempRock != None )
